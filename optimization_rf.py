...
class RfOptimizeMethod:
    
    '''Optimization base class calling other optimization technique namely Grid Search , 
    Bayes Opt and Hyper Opt. Here this class specifically defined for random forest with parameters
    as n_estimiators , max_depth, max_features , min_samples_leaf and min_sample_split. Cross Validation
...
if __name__ == "__main__":
    ...
    parser.add_argument("-max_features", nargs = '+', type = float)
    parser.add_argument("-min_samples_leaf", nargs = '+', type = float)
    parser.add_argument("-min_samples_split", nargs = '+' , type = float)
    ...
    parser.add_argument("-cross_val", "--cross_validation", help = 'used for grid search', nargs = '?', const = 3, type = int, default = 3)
    parser.add_argument("-o", "--option" , help = "enter the type of optimization method : grid_search, bayesopt, hyperopt")
    ...
    model_path = "/home/congnitensor/Python_projects/model_class/file_logs/model_rf.txt"
    # model_path = "model.txt"

    obj = RfOptimizeMethod(params = param_list, init_points = init_points , n_iter = num_iter , max_evals = max_evals, cross_val= cross_val, model_path= model_path) 
