...
from sample_bayes import CogniBayesOptimize
from sample_grid_search import CogniGridSearch
from hoptimize import CogniHyperOptimize 
import pickle
import argparse 


class XgbOptimizeMethod(DataPreprocessing,MetricsCal):
    ...
    def __init__(self,**kwargs):
        ...
        if 'feature_test' in kwargs:
            self.feature_test = kwargs['feature_test']
        if 'label_test' in kwargs:
            self.label_test = kwargs['label_test']  
    
    # Calling Grid search for XGBoost
    def grid_search(self):

        obj_1 = CogniGridSearch(self.params,self.feature_test,self.label_test,
                                model_optimize = self.model_path)
        obj_1.grid_search_cv(self.cross_val)
        obj_1.optimize_metrics()
...
if __name__ == "__main__":
    ...
    obj = XgbOptimizeMethod(param = param_list, init_points = init_points , 
                            n_iter = num_iter , max_evals = max_evals, 
                            cross_val= cross_val, model_path= model_path,
                            feature_test = None, label_test = None) 
    
    op_dict = {"gridsearch": obj.grid_search,
               "bayesopt":obj.bayes_opt,
               "hyperopt":obj.hyper_opt}
